...
// 是否禁用数据库（通过环境变量控制，本地可设置 DISABLE_DB=true）
const DB_DISABLED = process.env.DISABLE_DB === "true";

// 路由模块路径（按需加载，避免在禁用数据库时引入依赖数据库的路由）
...
// 仅在未禁用数据库时加载数据库模块
let getDatabase, initializeTables, closeDatabase;
if (!DB_DISABLED) {
  ({ getDatabase, initializeTables, closeDatabase } = require("./db/database"));
}
...
if (!DB_DISABLED) {
  ...
  console.warn("⚠️ 数据库已禁用：跳过挂载 /api/auth 与 /api/chat-sync 路由");
  // 提供轻量级的 verify 存根，避免前端调用 404
...
async function startServer() {
  try {
    ...
    if (!DB_DISABLED) {
      ...
      console.log("🚫 跳过数据库初始化（DISABLE_DB=true）");
    }
...
process.on("SIGTERM", () => {
  ...
  if (!DB_DISABLED && typeof closeDatabase === "function") {
    closeDatabase();
  }
  process.exit(0);
});
