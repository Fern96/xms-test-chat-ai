...
import KnowledgeBaseDrawer from "../components/ui/KnowledgeBaseDrawer";
...
const Home: React.FC = () => {
  ...
  return (
    <MainLayout
      onKnowledgeBaseClick={handleKnowledgeBase}
                  ...
                  <div className="text-foreground text-left break-words">
                    {message.isLoading ? (
                      ...
                      <div>
                        ...
                        {/* AI回复统计信息 */}
                        {message.stats && (
                          <div className="text-xs text-gray-400 dark:text-gray-500 mt-2 font-mono space-y-1">
                            <div>
                              {message.stats.model} | {message.stats.responseTime} | {message.stats.totalTokens} tokens
                            </div>
                            {message.stats.kbRefs && message.stats.kbRefs.length > 0 && (
                              <div className="text-[11px] text-gray-500 dark:text-gray-400">
                                引用：{message.stats.kbRefs.map((r, i) => `[#${r.index ?? i + 1}] ${r.title}`).join("; ")}
                              </div>
                            )}
                          </div>
                        )}
        ...
        <div className="p-6">
          <div className="w-[80%] mx-auto">
            ...
            <ChatInputPanel
              ...
              useKb={!!useKb}
              onToggleKb={(v) => setUseKb && setUseKb(v)}
            />
          </div>
        </div>

        {/* 底部工具栏 */}
        <div className="border-t border-surface bg-panel px-6 py-3">
          <div className="w-[80%] mx-auto flex items-center justify-center gap-6">
            <Button
              type="text"
            ...
            <Button
              ...
              onClick={() => message.info("连接数据库功能开发中")}
              className="flex flex-col items-center gap-1 h-auto py-2"
            >
              <span className="text-xs">连接数据库</span>
            </Button>
            <Button
              type="text"
              icon={<BulbOutlined />}
              onClick={() => message.info("智能建议功能开发中")}
              className="flex flex-col items-center gap-1 h-auto py-2"
            >
              <span className="text-xs">智能建议</span>
            </Button>
            <Button
              type="text"
